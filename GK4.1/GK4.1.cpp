// GK4.1.cpp : Defines the entry point for the console application.
//
...
enum
{
	...
	TEXTURE_SKINGIRL,                    // tekstura tan_skin_girl
	TEXTURE_SKINGIRL_UNC,                // teksura tan_skin_girl nieskompresowana
	TEXTURE_SKINGIRL_GRAY,               // tekstura tan_skin_girl_gray
	TEXTURE_SKINGIRL_GRAY_UNC,           // tekstura tan_skin_girl_gray nieskompresowana
...
GLuint SKINGIRL, SKINGIRL_UNC, SKINGIRL_GRAY, SKINGIRL_GRAY_UNC;
...
void GenerateTextures()
{
	...
	// wczytanie tekstury tan_skin_girl.tga
	GLboolean error = load_targa("blonde_guy_white_skin_face.tga", width, height, format, type, pixels);

	// b³¹d odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_guy_white_skin_face.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &SKINGIRL);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, SKINGIRL);

	// definiowanie tekstury z kompresj¹
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &SKINGIRL_UNC);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, SKINGIRL_UNC);
	...
	// wczytanie tekstury tan_skin_girl_gray.tga
	error = load_targa("blonde_guy_white_skin_face_gray.tga", width, height, format, type, pixels);

	// b³¹d odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_guy_white_skin_face_gray.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &SKINGIRL_GRAY);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, SKINGIRL_GRAY);

	// definiowanie tekstury z kompresj¹
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_LUMINANCE, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &SKINGIRL_GRAY_UNC);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, SKINGIRL_GRAY_UNC);
...
void Menu(int value)
{
	...
	// tekstura tan_skin_girl
	case TEXTURE_SKINGIRL:
	{
		texture = SKINGIRL;
		DisplayScene();
	}
	...
	// teksura tan_skin_girl nieskompresowana
	case TEXTURE_SKINGIRL_UNC:
	{
		texture = SKINGIRL_UNC;
		DisplayScene();
	}
	break;

	// tekstura tan_skin_girl_gray
	case TEXTURE_SKINGIRL_GRAY:
	{
		texture = SKINGIRL_GRAY;
		DisplayScene();
	}
	break;

	// tekstura tan_skin_girl_gray nieskompresowana
	case TEXTURE_SKINGIRL_GRAY_UNC:
	{
		texture = SKINGIRL_GRAY_UNC;
		DisplayScene();
	}
	break;
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("blonde_guy_white_skin_face.tga (kompresja)", TEXTURE_SKINGIRL);
	glutAddMenuEntry("blonde_guy_white_skin_face.tga (bez kompresji)", TEXTURE_SKINGIRL_UNC);
	glutAddMenuEntry("blonde_guy_white_skin_face_gray.tga (kompresja)", TEXTURE_SKINGIRL_GRAY);
	glutAddMenuEntry("blonde_guy_white_skin_face_gray.tga (bez kompresji)", TEXTURE_SKINGIRL_GRAY_UNC);
